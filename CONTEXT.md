...
**Disposition Table**:
One row per legacy option value across both Managed Fields — legacy identifier, legacy display text, chosen Profile Link value, target URL, Disposition, and the Custom User Field the value must be _written into_ — emitted by a **Catalogue Refresh**, committed, and read here by one release gate and nothing else — `pnpm check:collection-assignment` refuses a `collection-link-fault` row and asks the file nothing further, so nothing this repository builds or ships is derived from it. It is the entire interface to a separate, non-public repository, which joins it against a fresh member export to produce the three columns Discourse asked for: member identifier, custom field name, value. Which string a legacy value becomes is decided here; which member holds it is decided there, and that division is what keeps this repository free of personally identifiable information. Two rules run it. A row **with** a URL carries a value byte-identical to a Mapping shipped in `settings.yml` — stricter than the runtime's own trimmed match on purpose, because a one-byte difference in somebody else's join is a member whose Profile Link renders nothing, and that consumer gets none of the runtime's tolerance (ADR-0023). A row **without** one carries the legacy display text verbatim and no suffix, because the suffix is a Collection Link's anchor text (ADR-0020) and a value that is not a link has none: the member keeps what they entered as an **Unmatched Value** rather than losing it. Every legacy value gets a row, including the majority that resolve to a live product, because a table of only the Collection Links would be missing most of what the join needs while looking complete. The `destination_field_name` column follows from the Disposition and adds no rows: a `collection` row names the Managed Field's **Shadow Field**, because a Collection Link put into a `dropdown` is one profile save from `""`, and every other row names the Managed Field, whose value it can keep (ADR-0026). It is stated here rather than left to the far side to derive, because this is the entire interface and a rule the consumer cannot watch change is a rule the two sides drift on.
...
**Managed Field**:
One of the two Custom User Fields this pipeline is responsible for, as named by the Sheet Export allowlist — `Machine` and `Mask`. A Managed Field with no Mappings behind it is still in scope — that is how it gets reported rather than forgotten — while a field outside the list is never mentioned at all. `Humidifier` passed through both states — permanently unmapped under ADR-0012, briefly in scope via Collection Links under ADR-0020 — before being dropped from the pipeline entirely (ADR-0022): its Collection Link targets turned out to carry no real link equity on inspection, and the business no longer needs the field carried through this migration. Its Custom User Field may still exist on the live Discourse instance; that is a site-administration question this pipeline does not answer (ADR-0015). `Software` is a Custom User Field on the instance but was never a Managed Field, and nothing here links it. A **Shadow Field** is not one either, and that is the case worth stating because it is the one that exists to carry a Managed Field's own values: it is outside the allowlist by construction, a Catalogue Apply never writes it, and it is never given a Dropdown Option.
_Avoid_: known field, our field, target field

**Shadow Field**:
A `text`-typed Custom User Field that holds a **Collection Link** value durably, because a `dropdown` cannot. Discourse resolves an off-list `dropdown` value to `nil` on its holder's next profile save that submits the field, and a Collection Link is off-list permanently and by construction (ADR-0021, #58) — so the value the migration writes is one save from `""`, with nothing in this repository recording what it was. The `text` type is the whole of the immunity: `clean_custom_field_values` returns the `else` branch verbatim, with no option list to consult.

There is one per **Managed Field** — `Machine (Discontinued)` and `Mask (Discontinued)` — because a User can hold a discontinued machine _and_ a discontinued mask, and one field holds one string (ADR-0026). It holds only values a User cannot choose: a value still on the option list is not at risk, and a Shadow Field that mirrored one would go stale the moment its holder edited the field they _can_ edit, then render the wrong equipment the moment they cleared it. Absent is a Profile Link missing; stale is a Profile Link lying.

Its configuration is forced rather than chosen, and both halves were measured (ADR-0024). `editable: false` is the protection — `users_controller.rb` narrows the writable set with `fields.where(editable: true) unless current_user.staff?` before it looks at the payload, so no User's save can reach the field even carrying a valid value, while this pipeline's own staff-authenticated writes go straight through. `show_on_profile` and/or `show_on_user_card`, matched to the **Link Surfaces** it feeds, is the half that bites: `allowed_user_field_ids` branches on the _reader_, so a field hidden from both is **absent** from an anonymous payload rather than null — and it still reads back fine for staff and for the profile's owner, which is why the misconfiguration tests clean for whoever built it.

A Link Surface reads it only when the Managed Field it shadows holds nothing at all, and labels the resulting **Profile Link** with the _Managed Field's_ name. Holding nothing is narrower than resolving nothing: a Managed Field holding an **Unmatched Value** still wins, because that is a value the User has now and the Shadow Field's is one they cannot correct. The fallback changes where the value came from, never which link it is.
_Avoid_: backup field, mirror field, hidden field, durable field, retained field, second field
...
**Four things are called "discontinued", and only one is a fact about a product.**
...
The fourth is a **Shadow Field**'s own name — `Machine (Discontinued)`, `Mask (Discontinued)` — and it describes the _field_, not any value in it. A Shadow Field holds Collection Link values, which each carry the suffix themselves, so its stored strings end in the word twice over for two unrelated reasons. Never read a Shadow Field's name as a claim about a product, and never let a Managed Field's name and its Shadow Field's name be treated as the same field spelled differently: `Machine` is in the Sheet Export allowlist and `Machine (Discontinued)` never is.
