...
describe("the disposition table this repository commits", () => {
  ...
  it("holds no cell shaped like an email address", () => {
    ...
    //
    // Imported rather than copied. `EMAIL_SHAPED`'s own docblock is about
    // exactly this: "a guard that had drifted between the two directions would
    // be strict at one boundary and lax at the other, and nobody would find
    // out which." A second copy here meant tightening the guard left this
    // committed-file gate silently on the old rule.
    const shaped = EMAIL_SHAPED;
